...
import { IonicPage, AlertController, ActionSheetController, ActionSheet, NavController, NavParams } from 'ionic-angular';
...
@IonicPage({
	name: 'investment-detail-1'
})

@Component({
	selector: 'page-investment-detail-1',
	templateUrl: 'investment-detail-1.html'
})
export class InvestmentDetailPage1 {
	...
	constructor(public actionSheetCtrl: ActionSheetController, public alert: AlertController,
		...
		public navParams: NavParams, youtube: YoutubeVideoPlayer, public modalCtrl: ModalController,
		imageViewerCtrl: ImageViewerController, public session: SessionProvider, private photoViewer: PhotoViewer) {

		this.primeObjects();

		this.legalId = 26; //THIS IS THE INVESTMENT OPPORTUNITY LEGAL ID
	...
	private getLatestVitals() {

		this.user.requestProductVitals(this.legalId).then(async (response) => {

			if (response == null || !response) {
				console.log("null response calling session.exitToLoginPage()");
				this.session.exitToLoginPage().then((result) => {
					console.log("Called 1");
					this.navCtrl.setRoot(AuthPage, { "sessionExpired": true });
				}, () => {
			...
			if (response && response.data) {
				if (response.data.state === 0) {
					this.session.exitToLoginPage(); return;
				}
	...
	private viewProductImage(id:number){
		console.log(id);
		...
		this.photoViewer.show(this.product_images[id].src, this.product_images[id].description, options);
		return;
